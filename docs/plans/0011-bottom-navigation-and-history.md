...
| Phase | Steps | Status |
|---|---|---|
| 1. Backend — offer on a conversation | 3 / 3 | Done |
| 2. Backend — the history endpoint | 2 / 2 | Done |
| 3. The tab bar | 4 / 4 | Done |
| 4. Top bar and routing | 3 / 3 | Done |
| 5. The History page | 2 / 2 | Done |
| 6. Clearing the bar | 3 / 3 | Done |
...
- [x] **Action.** Add to `conversations` in
...
- [x] **Action.** Add an index on `offerId`, then `db:generate` and `db:migrate`.
- [x] **Verify.** The migration only adds a nullable column, a foreign key and an
  index.

### Step 1.2 — Accept it when starting a conversation

- [x] **Action.** Let the start-conversation endpoint take an optional
...
- [x] **Verify.** An offer belonging to a different creative is a 400. Omitting
  `offerId` still works and stores null.

### Step 1.3 — Send it from the offer page

- [x] **Action.** `OfferDetailPage` and the offer cards on the profile page pass
...
- [x] **Verify.** Contacting from an offer stores the id; contacting from a
  profile stores null.
...
- [x] **Action.** Behind `requireAuth`, return the caller's conversations **as
...
- [x] **Action.** Resolve offers and creatives for the whole page in one query
  each, keyed by id — the pattern `subdomainsForProfiles` already uses.
- [x] **Verify.** A conversation whose offer was deleted still appears, with no
...
- [x] **Action.** Create `frontend/src/components/BottomNav.tsx`. Renders only
...
- [x] **Verify.** It appears below `sm` when signed in, and nowhere else.

### Step 3.2 — Touch targets and safe area

- [x] **Action.** Each tab is at least 44px tall and fills an equal quarter of
  the width. Add `pb-[env(safe-area-inset-bottom)]` so the bar is not sliced by
  the home indicator on a notched phone.
- [x] **Verify.** On a 320px-wide viewport all four labels fit without wrapping
  or truncating.

### Step 3.3 — The unread badge

- [x] **Action.** Messages carries the unread count from `useUnreadCount`, the
  same source the header uses. Render `99+` above 99.
- [x] **Verify.** The badge matches the header's on desktop.

### Step 3.4 — Mount it

- [x] **Action.** Render `<BottomNav />` once, next to `<ToastProvider>` in
  `App.tsx`, rather than per page. Adding it per page guarantees a page is
  eventually missed.
- [x] **Verify.** It shows on every signed-in route, including offer and
...
- [x] **Action.** Below `sm`, `SiteHeader` shows only the logo and the bell. The
  links move to the tab bar. Above `sm` it is untouched.
- [x] **Verify.** At 400px the header holds the logo and the bell and nothing
  else. At 1024px it is exactly as before.

### Step 4.2 — The bell

- [x] **Action.** A `Bell` icon in muted ink. **A `<span>`, not a `<button>`** —
...
- [x] **Verify.** Tapping it does nothing and shows no affordance. A screen
  reader skips it.

### Step 4.3 — Home

- [x] **Action.** The Home tab routes to `/directory`. Signed-out visitors keep
...
- [x] **Action.** Create `frontend/src/pages/HistoryPage.tsx` at `/history`,
...
- [x] **Action.** Where a conversation has no offer, show the creative and
  "Contacted from their profile". Rule 4 — this is normal, not a gap.
- [x] **Verify.** Both shapes render correctly.

### Step 5.2 — The empty state

- [x] **Action.** Nobody has inquired about anything yet, so this is the common
  case. Say so plainly and link to the directory.
- [x] **Verify.** A fresh account sees the empty state, not a blank page.
...
- [x] **Action.** Add bottom padding below `sm` to every scrollable page, sized
  to the bar plus the safe-area inset.
- [x] **Verify.** On a phone viewport, the last row of the directory, the
  messages list and the history list are all fully visible and tappable.

### Step 6.2 — The toast stack

- [x] **Action.** `ToastProvider`'s stack is `fixed bottom-0`. Raise it above
  the bar on small screens.
- [x] **Verify.** Save an offer on a phone viewport: the toast sits above the
  bar, covering neither it nor the page's last row.

### Step 6.3 — The composer

- [x] **Action.** Check `ConversationPage`'s message input, which sits at the
  bottom of a chat. It must not end up underneath the bar.
- [x] **Verify.** The input is reachable and usable with the keyboard open.
...
- [x] **Verify.** Every row is reachable without typing a URL.

### Step 7.2 — Ownership

- [x] **Verify.** Starting a conversation with an `offerId` belonging to another
  creative is a 400.
- [x] **Verify.** `GET /conversations/history` returns only the caller's own
  conversations.

### Step 7.3 — Deleted offers

- [x] **Verify.** Delete an offer that has a conversation. The conversation and
  its messages survive, and History shows it without an offer.

### Step 7.4 — Desktop is untouched

- [x] **Verify.** At 1024px there is no bottom bar and the header is unchanged,
...
- [x] A signed-in user on a phone has Home, Messages, History and Profile one
      tap away
- [x] The phone header is the logo and a non-interactive bell
- [x] Desktop navigation is unchanged
- [x] The mode switch and Sign out are reachable on a phone
- [x] Conversations started from an offer record it; others store null
- [x] History lists inquiries with reply status, and survives a deleted offer
- [x] No page has content or a toast trapped behind the bar
