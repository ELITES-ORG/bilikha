...
that happened to you. It covers events that have no other home — moderation
outcomes and posting replies — and leaves messages to the badge they already
have. The agreement types join the same enum when
[plan 0016](./0016-work-agreements.md) lands.
...
| Phase | Steps | Status |
|---|---|---|
| 1. Schema | 2 / 2 | Complete |
| 2. Service | 4 / 4 | Complete |
| 3. Emit points | 2 / 2 | Complete |
| 4. API | 1 / 1 | Complete |
| 5. The bell and the centre | 4 / 4 | Complete |
| 6. Copy | 1 / 1 | Complete |
...
- [x] **Action.** `backend/src/db/schema/notifications.ts`:
...
- [x] **Why.** The partial index is what keeps the polled count cheap. Without
  it, every poll from every signed-in user scans that user's whole history.
- [x] **Note.** There is no `title` or `body` column. Rule 1.

### Step 1.2 — Migration

- [x] **Action.** `npm run db:generate`, read the SQL, commit the snapshot with
...
- [x] **Action.** `notify({ userId, actorUserId, type, targetId })`. Returns
  nothing useful and never throws: wrap the insert, log a failure, carry on.
  Rule 4.
- [x] **Action.** It refuses silently when `userId === actorUserId`. Rule 3 —
  enforce it in one place rather than at every call site.

### Step 2.2 — Resolving targets

- [x] **Action.** `resolveTargets(rows)` batches one lookup per target type and
  returns a title and a link for each notification. Never one query per row.
- [x] **Action.** Anything that does not resolve — deleted, or owned by a
  suspended account — returns a tombstone: "This is no longer available." It
  stays in the list, it is not silently dropped.
- [x] **Action.** Every lookup that reaches another user's content filters
  `users.status = 'active'`, per
  [ADR 0028](../decisions/0028-suspension-is-enforced-per-request.md).
- [x] **Verify.** Suspend an account that is the actor on a notification and
  confirm the recipient sees a tombstone, not their name or their text.

### Step 2.3 — Reading

- [x] **Action.** `listNotifications(userId, { page, limit })`, newest first.
...
- [x] **Action.** `backend/src/scripts/prune-notifications.ts` plus a
  ...
  `--commit`.
- [x] **Why.** ADR 0030 names unbounded growth as a known cost. A script that
...
- [x] **Action.** In `admin.service.ts`, `moderate()` emits
...
- [x] **Action.** Do not emit for `account_suspended` or `account_reinstated`. A
...
- [x] **Action.** Where a creative's reply to a posting creates or continues a
  conversation, emit `posting_replied` to the client who posted, targeting the
  conversation.
- [x] **Action.** Emit once per conversation, not once per reply. Check for an
...
- [x] **Action.** Mount at `/api/v1/notifications` behind `requireAuth`:
  - `GET /` — paginated list, resolved
  - `GET /unread-count` — `{ count }`, nothing else
  - `POST /:id/read`
  - `POST /read-all`
- [x] **Action.** `/unread-count` does the least work of any endpoint in the
...
- [x] **Action.** In `SiteHeader`, replace the `aria-hidden` `<span>` with a
  `Link` to `/notifications`, labelled "Notifications", showing the unread count
  as a `Badge` when non-zero.
- [x] **Action.** Remove the `sm:hidden`. The bell belongs on every breakpoint —
  it is currently hidden on desktop, where there is no other route to it.
- [x] **Verify.** It is reachable by keyboard and announces its count.

### Step 5.2 — The query

- [x] **Action.** `useNotificationCount()`, following `useUnreadCount`:
  `refetchOnWindowFocus`, and an interval of about 60s that is disabled when
  `document.visibilityState !== 'visible'`. Rule 5.
- [x] **Action.** Do not add a second poller. If both counts end up polling,
  fetch them in one request rather than two.

### Step 5.3 — The centre

- [x] **Action.** `/notifications`, guarded. Rows: actor's avatar where there is
  an actor, the resolved title, relative time, and an unread dot. Tapping marks
  it read and follows its link.
- [x] **Action.** "Mark all as read" in the header. `toast.run`, like every
  other action.
- [x] **Action.** An empty state in the voice of the existing ones.
- [x] **Action.** `pbBottomNav` for tab-bar clearance.

### Step 5.4 — Tombstones

- [x] **Action.** An unresolved target renders greyed, is not a link, and says
...
- [x] **Action.** `MessagesPage` currently reads "Threads with creatives and
  ...
  contradiction.
- [x] **Action.** Grep the whole frontend for other copy promising no alerts or
...
- [x] Approve a profile as an admin; its owner's bell shows 1 and the centre
  names the profile. Reject another; same.

### Step 7.2 — It is not yours

- [x] The acting admin gets nothing. Rule 3.
- [x] `POST /notifications/:id/read` with another user's notification id: refused,
  and their notification stays unread.

### Step 7.3 — Suspension

- [x] Suspend an actor and confirm the recipient's row becomes a tombstone with
  no name and no content. Reinstate and confirm it comes back. ADR 0028.

### Step 7.4 — It does not break its cause

- [x] Make `notify` throw deliberately. The moderation decision still commits and
  the admin sees success. Rule 4.

### Step 7.5 — Polling behaviour

- [x] With the tab hidden, confirm in the network panel that no count requests
  are made. Bring it forward and confirm it refetches. Rule 5.

### Step 7.6 — Full pass

- [x] `npm run typecheck`, `lint`, `build`, `docs:check` all exit 0. Grep the
