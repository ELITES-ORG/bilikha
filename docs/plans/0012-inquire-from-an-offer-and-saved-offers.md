...
| 2. Backend — messaging | 3 / 3 | Done |
| 3. Backend — saved offers and history | 3 / 3 | Done |
| 4. Frontend — the offer page | 3 / 3 | Done |
| 5. Frontend — the thread | 3 / 3 | Done |
| 6. Frontend — History | 2 / 2 | Done |
...
- [x] **Action.** Add `offerId` to `messages`, nullable,
  `references(() => offers.id, { onDelete: 'set null' })`, with an index. A
  deleted offer must not delete the message.
- [x] **Action.** Add `saved_offers`: `userId` and `offerId`, both cascading,
...
- [x] **Action.** Hand-write the migration that follows the generated one, in
...
- [x] **Verify.** Seed a conversation with an offer and two messages. After the
...
- [x] **Action.** Drop `subject` from the start-conversation body. Accept an
  optional `offerId` and **store it on the message, not the conversation**.
- [x] **Action.** Keep the ownership check from plan 0011: the offer must belong
  to the creative being contacted, or 400. Without it, a card could be attached
  claiming an inquiry that never happened.
- [x] **Verify.** Starting without an offer works. An offer belonging to someone
  else is 400.

### Step 2.2 — Continuing one

- [x] **Action.** `POST /conversations/:id/messages` also takes an optional
...
- [x] **Verify.** Two messages in one thread can carry two different offers, and
  both survive.

### Step 2.3 — Serving the card

- [x] **Action.** Message payloads include the offer when present: id, title,
  price, first image, and whether it still exists.
- [x] **Action.** Resolve offers for a page of messages in **one** query keyed
  by id. Never per message.
- [x] **Verify.** A thread of 50 messages issues a fixed number of queries.
...
- [x] **Action.** `POST /me/saved-offers` with `{ offerId }`, and
...
- [x] **Action.** Only published offers can be saved.
- [x] **Verify.** Saving twice leaves one row. Unsaving something not saved is a
  204, not an error.

### Step 3.2 — History

- [x] **Action.** Rewrite `GET /conversations/history` to read from messages:
...
- [x] **Action.** Add `GET /me/saved-offers` returning saved offers with the
  creative, newest first.
- [x] **Verify.** Asking about one offer three times is one row. Asking about
  two offers from the same creative is two rows — the case the old column could
  not represent.

### Step 3.3 — Reference

- [x] **Action.** Update [`api.md`](../reference/api.md): the removed `subject`,
...
- [x] **Action.** At the bottom of `OfferDetailPage`: **Inquire** (primary) and
...
- [x] **Verify.** Signed out, both go to login and return to the offer.

### Step 4.2 — Save is a toggle

- [x] **Action.** Save reflects current state — "Save" or "Saved" — and toggles.
  Use the toast helper so it reports pending, done and failed like every other
  action.
- [x] **Verify.** Saving, reloading, and unsaving all hold.

### Step 4.3 — Inquire

- [x] **Action.** Inquire opens the thread with that creative — existing or new
  — with the offer attached to the composer. It does **not** send anything on
  its own.
- [x] **Verify.** A client with an existing thread lands in it with the card
...
- [x] **Action.** The composer shows the attached offer above the input, with a
  way to detach it before sending.
- [x] **Verify.** Detaching sends a plain message.

### Step 5.2 — The card

- [x] **Action.** Render a message's offer as a card above its body: thumbnail,
  title, price, linking to the offer. Where the offer is gone, show "This offer
  is no longer listed" and keep the body.
- [x] **Verify.** Delete an offer with a card in a thread. The message survives
  and reads sensibly.

### Step 5.3 — No more subject

- [x] **Action.** `MessagesPage` shows the other person's name, their avatar,
  and the last message. Remove the subject from `ConversationPage` too.
- [x] **Verify.** Threads are still distinguishable at a glance.
...
- [x] **Action.** `HistoryPage` gets **Inquired** and **Saved**. Inquired is the
  default. Put the selected segment in the URL so a reload keeps it.
- [x] **Verify.** Switching segments and reloading holds the choice.

### Step 6.2 — Empty states

- [x] **Action.** Each segment gets its own, both linking to the directory.
  Nobody has saved or inquired about anything yet, so these are the common case.
- [x] **Verify.** A fresh account sees both, not a blank page.
...
- [x] **Verify.** As a client, inquire about offer A from a creative, then offer
  B from the same creative. One thread, two cards, **two rows in History**, and
  A is still there.

### Step 7.2 — Migration

- [x] **Verify.** Every conversation that had an `offer_id` has it on its
  earliest client message.
- [x] **Verify.** `conversations.offer_id` and `conversations.subject` are gone.
- [x] **Verify.** A deliberately broken move aborts the migration rather than
...
- [x] **Verify.** Deleting an offer leaves its messages intact, removes it from
...
- [x] Inquiring about two offers from one creative preserves both
- [x] Inquire attaches the card to the client's own message; nothing auto-sends
- [x] Subject is gone from the schema, the API and the UI
- [x] Offers can be saved and unsaved, and saving twice does not duplicate
- [x] History has Inquired and Saved, grouped by offer
- [x] A deleted offer leaves its messages readable
- [x] The migration moves data before dropping, and refuses to drop otherwise
...
| Sweeping abandoned conversations | `POST /conversations/ensure` creates the row before anything is said, so tapping Inquire and leaving writes a message-less row. It is hidden from both inboxes, but it still accumulates. Either create the conversation on first send, or prune rows with no messages |
