...
- [x] **Action.** Add to `backend/src/db/schema/agreements.ts`:
...
- [x] **Why.** Acceptance is a separate table because it is a different kind of
...
- [x] **Why.** The events table exists because an accepted agreement is frozen
...
- [x] **Note.** There is no `current_status` column anywhere. Rule 9.

### Step 1.2 — The attachment column

- [x] **Action.** Add `agreementId` to `messages` in
...
- [x] **Note.** ADR 0029 fixes the line at three attachment columns. Do not add
  a fourth in this plan or any other without revisiting that decision.

### Step 1.3 — Constraints and migration

- [x] **Action.** Add check constraints: `duration_days > 0`,
  `price_centavos >= 0`, `version > 0`.
- [x] **Action.** Add a trigger, or a `BEFORE UPDATE` rule, that raises when a
...
- [x] **Action.** `npm run db:generate`, then read the generated SQL before
...
- [x] **Verify.** `npm run db:migrate` against a local database, then
...
- [x] **Action.** `canonicalContent(agreement, lineItems)` returns a
...
- [x] **Action.** `contentHash(...)` — SHA-256 hex of that string, via `node:crypto`.
- [x] **Why.** The hash must be reproducible from the stored rows alone. Anything
...
- [x] **Action.** `issueAgreement(userId, conversationId, input)`. Refuses unless
...
- [x] **Action.** `input` requires at least one line item and a package title;
  `startDate` is a date, `durationDays` a positive integer.
- [x] **Verify.** A client calling this gets 403. A stranger gets 404, matching
  `requireParticipant`.

### Step 2.3 — Revise

- [x] **Action.** `requestRevision(userId, agreementId, note)`. Client only.
...
- [x] **Action.** `issueAgreement` accepts an optional `supersedesId`. When
...
- [x] **Why.** Marking the predecessor inside the transaction is what stops two
  live versions existing. Doing it in a second statement leaves a window where
  the client can accept the one being replaced.

### Step 2.4 — Accept

- [x] **Action.** `acceptAgreement({ userId, agreementId, password, seenHash })`:
...
- [x] **Action.** Do the hash check *before* the password check. A client whose
  document changed should be told that, not asked for a password first.
- [x] **Verify.** `password` appears in no log line, no error body, and no
  returned object. Grep the module for it before moving on.

### Step 2.5 — Read

- [x] **Action.** `getAgreement(userId, agreementId)` for either participant, and
  a card loader `loadAgreementCards(ids)` matching `loadOfferCards`.
- [x] **Action.** Both compute `totalCentavos` as the sum of line items and
  `endDate` as `startDate + durationDays`. Neither is read from a column.
- [x] **Action.** Extend `messageAttachmentFields` to branch three ways, adding
  `agreement` and `agreementRemoved`.
- [x] **Action.** Every read also returns the derived lifecycle state from
  Step 2.6, with the timestamp and actor of the event that set it.

### Step 2.6 — The lifecycle

- [x] **Action.** `deriveState(agreement, events)` — a pure function, no database
...
- [x] **Action.** `recordEvent({ userId, agreementId, type, note })` validates
...
- [x] **Action.** Insert the event and post a message into the thread in one
  transaction, so the conversation shows the move.
- [x] **Action.** Emit a notification to the *other* party. The plumbing is
  ...
  `/agreements/:id`.
- [x] **Action.** Call it *after* the transaction commits, as
  `admin.service.ts` `notifyOwner` does. It swallows its own failures, so it
  cannot affect the transition either way.
- [x] **Note.** `notify` already drops self-notifications, so the actor never
  needs filtering at this call site.
- [x] **Action.** Take `pg_advisory_xact_lock(hashtext(agreementId))` before
...
- [x] **Verify.** No code path anywhere sets a state from `now()` or from
...
- [x] **Action.** Mount `agreementsRouter` at `/api/v1/agreements` in
...
- [x] **Action.** Validate every id with `z.string().uuid()`. Validate line items
  with a max count — 30 is generous — and a max description length.

### Step 3.2 — Rate limiting

- [x] **Action.** Add `agreementAcceptLimiter` to
  ...
  attempts per hour.
- [x] **Why.** This endpoint takes a password. Without a limiter it is a
...
- [x] **Action.** `frontend/src/features/agreements/AgreementComposer.tsx`:
  package title, repeatable line items (description + peso input), notes, start
  date, duration in days.
- [x] **Action.** Peso inputs use `groupPesoDigits` and `pesoInputToCentavos`
  from `@/lib/money`, as offers and postings do.
- [x] **Action.** Show the running total and the computed end date live, both
  derived in the component. Never send either to the server.

### Step 4.2 — Where it opens

- [x] **Action.** The creative's thread gets an action to draft an agreement. It
...
- [x] **Action.** On a `sent` agreement showing a revision request, the same
  composer opens prefilled from that version, and sends with `supersedesId`.

### Step 4.3 — Feedback

- [x] **Action.** Every submit goes through `toast.run`, as all actions do.
...
- [x] **Action.** An agreement card in the thread, alongside the offer and
...
- [x] **Action.** The card links to `/agreements/:id`, built in Phase 6. There is
  no second full-agreement view inside the thread — one record, one address.
- [x] **Why.** Two renderings of the same document drift, and the one people
  screenshot is whichever they happened to open.

### Step 5.3 — Accept

- [x] **Action.** The client sees two actions: **Request changes** (a note) and
  **Accept**. Accept opens a confirmation asking for their password, carrying the
  content hash the view rendered.
- [x] **Action.** The confirmation states plainly: this records that you agreed
  to these terms. It is not a legal signature, and Bilikha does not handle
  payment. ADR 0029.
- [x] **Action.** Add a line of help text: Bilikha will never ask for this
...
- [x] **Action.** The card becomes Accepted and both action buttons disappear for
...
- [x] **Action.** On the record page, show only the action the current state and
...
- [x] **Action.** Awaiting confirmation shows the creative a plain line: waiting
  for the client to confirm. Not a button they can press. ADR 0029.
- [x] **Action.** No password on any of these. Only acceptance takes one.
- [x] **Action.** Every state on screen carries when it was set and by whom —
  "In progress · marked by Ana, 3 Oct". A four-month-old state must read as old,
  not as current.
- [x] **Action.** Everything goes through `toast.run`.

### Step 6.4 — The index

- [x] **Action.** Add `agreements` as a third segment in `HistoryPage`, beside
  `inquired` and `saved`, following the existing `HistorySegment` type, the
  `?segment=` param and the `role="tablist"` markup already there.
- [x] **Action.** Mirror by mode, as that page already does: in *I'm for hire* it
...
- [x] **Action.** Each row: the other party, package title, total, status chip,
  and a schedule line derived from the dates only — "Starts 3 Oct", "Ended
  14 Nov". Rule 8.
- [x] **Action.** Sort awaiting-response first, then by start date.

### Step 6.5 — The backend for it

- [x] **Action.** `listAgreements(userId, mode)` in the agreements service,
  returning rows for whichever side the mode names. It filters on the caller
  being a participant, not on a slug or a profile.
- [x] **Action.** `GET /api/v1/agreements?mode=` behind `requireAuth`.
- [x] **Verify.** The list query joins `users` and filters `status = 'active'`
...
- [x] Creative issues an agreement with three line items; client opens it, sees
...
- [x] Accept with a wrong password: refused, agreement still `sent`, no
  acceptance row, and the response says nothing about the agreement.

### Step 7.3 — The changed document

- [x] Open the accept screen, issue a superseding version from another session,
  then accept: refused on the hash check, and the client is told to review again.

### Step 7.4 — Immutability

- [x] After acceptance, attempt to update the agreement and its line items
...
- [x] A creative cannot accept their own agreement. A client cannot issue one. A
  third account gets 404 on every route for that agreement, not 403.

### Step 7.6 — The lifecycle, in order

- [x] Accept an agreement, then walk it: creative marks started, creative marks
...
- [x] The creative cannot confirm completion of their own work — refused
  server-side, not merely hidden in the UI.
- [x] The client cannot mark started or delivered.
- [x] Nothing can be recorded against an agreement that is still `sent`.
- [x] Nothing can follow Completed or Cancelled.
- [x] Cancelling without a reason is refused.
- [x] Fire two transitions concurrently against one agreement; exactly one
  lands. Without the advisory lock both do.
- [x] Wind a local agreement's `start_date` into the past and confirm the state
...
- [x] **Test.** Fire two `recordEvent` calls for the same agreement
...
- [x] **Why.** This is the single hardest thing in the plan to get right and the
